...
![RunLoop数据结构关系图](https://github.com/xiu619544553/dailyCode/blob/master/runloop结构体.png)
...
* 不同 Mode 中的 Source0/Source1/Timer/Observer能分隔开来，互不影响；
...
我们一起探讨 `CFRunLoopAddTimer` 函数实现：`CFRunLoopAddTimer()`函数中会判断传入的modeName模式名称是不是 `kCFRunLoopCommonModes` 通用模式，是的话就会将timer添加到`RunLoop` 的 `_commonModeItems` 集合中，并同步该timer到 `_commonModes` 里的所有模式中，这样无论在默认模式还是界面追踪模式下NSTimer都可以执行。
