...
struct SideTable {
    // 自旋锁
    spinlock_t slock;
    // 引用计数的 hash 表（协助对象的 isa 指针的 extra_rc 共同引用计数的变量）
    RefcountMap refcnts;
    // weak 引用全局 hash 表
 ...
 *
 * 这个函数将一个新值存储到__weak变量中。它将可在__weak变量作为赋值目标的任何地方使用。
 * 
...
public:
    // SIZE：每个 Page 对象占用 4096 个字节内存。为什么是4096呢？其实就是虚拟内存每个扇区4096个字节
	static size_t const SIZE =
#if PROTECT_AUTORELEASEPOOL
		PAGE_MAX_SIZE;  // must be multiple of vm page size。必须是虚拟机页面大小的倍数
#else
		PAGE_MIN_SIZE;  // size and alignment, power of 2
#endif
    
private:
	static pthread_key_t const key = AUTORELEASE  _POOL_KEY;
    ...
    // never uses them.
#   define EMPTY_POOL_PLACEHOLDER ((id*)1) // 表示一个空自动释放池的占位符
...
public:
    ...
    pop(void *token)
    {
        ...
        id *stop;
        if (token == (void*)EMPTY_POOL_PLACEHOLDER) { // 如果 token 是 EMPTY_POOL_PLACEHOLDER，就清空这个自动释放池
