...
#else
        PAGE_MAX_SIZE;  // size and alignment, power of 2   大小和排列，2的幂
#endif
    static size_t const COUNT = SIZE / sizeof(id);

    magic_t const magic;                // 用于校验当前 AutoreleasePoolPage 的完整性校验
    id *next;                           // next指针指向当前页中下一个可以存储变量的地址
    pthread_t const thread;             // 当前所在的线程。AutoreleasePool是按线程一一对应的（结构中的thread指针指向当前线程）
    AutoreleasePoolPage *const parent;  // 父节点，指向前一个 AutoreleasePoolPage
    AutoreleasePoolPage *child;         // 子节点，指向下一个 AutoreleasePoolPage
    uint32_t const depth;               // 深度
    uint32_t hiwat;                     // 全拼: high water，用来计算pool中最多存放的对象个数（最大入栈的数量）。在每次执行pop()的时候，会更新一下这个字段
