...
struct __CFRunLoopMode {
    ...
    Boolean _stopped;
    char _padding[3];
    CFMutableSetRef _sources0;    // 存储 __CFRunLoopSource
    CFMutableSetRef _sources1;    // 存储 __CFRunLoopSource
    CFMutableArrayRef _observers; // 存储 __CFRunLoopObserver
    CFMutableArrayRef _timers;    // 存储 __CFRunLoopTimer
...
CF_EXPORT CFRunLoopRef _CFRunLoopGet0(pthread_t t) {
    ...
    // 获取线程 t对应的 CFRunLoopRef
    CFRunLoopRef loop = (CFRunLoopRef)CFDictionaryGetValue(__CFRunLoops, pthreadPointer(t));
    __CFSpinUnlock(&loopsLock);
    
    // 如果线程 t没有对应的runloop，则创建一个添加到全局 __CFRunLoops
    if (!loop) {
...
static Boolean __CFRunLoopDoBlocks(CFRunLoopRef rl, CFRunLoopModeRef rlm) { // Call with rl and rlm locked
    ...
    while (item) {
        ...
        if (CFStringGetTypeID() == CFGetTypeID(curr->_mode)) {
            // 为什么
            // CFEqual(curr->_mode, curMode)：timer 加入的mode 与当前的 curMode 相同
            // 加入的Model curr->_mode = kCFRunLoopCommonModes，当前
 ...
 *  @param rlm             运行的mode
 *  @param seconds         runloop超时时间
 *  @param stopAfterHandle true:runloop处理完事件就退出  false:一直运行直到超时或者被手动终止
...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
    ...
    do {
        ...
        // 第一次do..whil循环不会走该分支，因为 didDispatchPortLastTime 初始化是true
        ...
        // 这里有个内循环，用于接收等待端口的消息
        // 进入此循环后，线程进入休眠，直到收到新消息才跳出该循环，继续执行 runloop
        do {
...
#pragma mark - 用指定的Mode启动，允许设置RunLoop超时时间
...
#pragma mark - 将 timer 添加到 runloop
void CFRunLoopAddTimer(CFRunLoopRef rl, CFRunLoopTimerRef rlt, CFStringRef modeName) {
    ...
    if (modeName == kCFRunLoopCommonModes) { // timer 添加到了 common 属性的 modes中
        // 将 _commonModes 拷贝到一个新的集合中
        CFSetRef set = rl->_commonModes ? CFSetCreateCopy(kCFAllocatorSystemDefault, rl->_commonModes) : NULL;
        if (NULL == rl->_commonModeItems) {
            rl->_commonModeItems = CFSetCreateMutable(kCFAllocatorSystemDefault, 0, &kCFTypeSetCallBacks);
        }
        // 将 timer 添加到 commonModeItems 中
...
/// 创建 CFRunLoopTimerRef
/// @param allocator 用于分配对象的内存
/// @param fireDate 在什么是触发 (距离现在)
/// @param interval 每隔多少时间触发一次
/// @param flags 未来参数
/// @param order CFRunLoopObserver的优先级 当在Runloop同一运行阶段中有多个CFRunLoopObserver 正常情况下使用0
/// @param callout 回调,比如触发事件,我就会来到这里
/// @param context 上下文记录信息
