...
- (void)viewDidLoad {
    ...
    /*
     ...
     官方文档：
     这个属性的默认值是YES，它让容器视图控制器知道他们应该调整这个视图控制器视图的滚动视图插入，以考虑状态栏、搜索栏、导航栏、工具栏或标签栏所消耗的屏幕区域。如果您的视图控制器实现管理自己的滚动视图插入调整，则将此属性设置为NO。
     */
...
#pragma mark - 结果
    ...
    NSLog(@"\ncontentInset: %@", NSStringFromUIEdgeInsets(self.tableView.contentInset));
    NSLog(@"\nnavigationBarFrame: %@", NSStringFromCGRect(self.navigationController.navigationBar.frame));
