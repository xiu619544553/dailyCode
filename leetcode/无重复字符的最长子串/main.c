...
/*
 ...
 滑动窗口讲解：
 因为题目告知我们的字符串由字母、数字、符合、空格组成，所以我们使用一个128（保证ASCII码中的所有元素都可以被记录）大小的int型数组int map[128]来记录遍历到的字符串中每一个字符的位置，由于map初始化为{0}，为了避免第一个字母的位置和初始化的0产生歧义，所以我们记录字符串的位置从1开始，即1~n。
